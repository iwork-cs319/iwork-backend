...
func (p PostgresDBStore) GetAllOfferings() ([]*model.Offering, error) {
	sqlStatement := `SELECT id, user_id, workspace_id, start_time, end_time, cancelled, created_by, locked FROM offerings;`
	return p.queryMultipleOfferings(sqlStatement)
}

func (p PostgresDBStore) GetAllExpandedOfferings() ([]*model.ExpandedOffering, error) {
	sqlStatement := `SELECT o.id, u.id, w.id, o.start_time, o.end_time, o.cancelled, o.created_by, o.locked, w.name, u.name, f.id, f.name
					 FROM offerings AS o
         			 INNER JOIN users AS u ON o.user_id = u.id
         			 INNER JOIN workspaces AS w ON o.workspace_id = w.id
         			 INNER JOIN floors AS f ON w.floor_id = f.id  
...
func (p PostgresDBStore) GetOfferingsByWorkspaceID(id string) ([]*model.Offering, error) {
	sqlStatement :=
		`SELECT id, user_id, workspace_id, start_time, end_time, cancelled, created_by, locked FROM offerings WHERE workspace_id=$1;`
...
func (p PostgresDBStore) GetOfferingsByUserID(id string) ([]*model.Offering, error) {
	sqlStatement :=
		`SELECT id, user_id, workspace_id, start_time, end_time, cancelled, created_by, locked FROM offerings WHERE user_id=$1;`
	return p.queryMultipleOfferings(sqlStatement, id)
}
...
func (p PostgresDBStore) GetOfferingsByDateRange(start time.Time, end time.Time) ([]*model.Offering, error) {
	sqlStatement :=
		`SELECT id, user_id, workspace_id, start_time, end_time, cancelled, created_by, locked FROM offerings 
...
func (p PostgresDBStore) GetOfferingsByWorkspaceIDAndDateRange(id string, start time.Time, end time.Time) (*model.Offering, error) {
	sqlStatement :=
		`SELECT id, user_id, workspace_id, start_time, end_time, cancelled, created_by, locked
		 FROM offerings
...
func (p PostgresDBStore) CreateOffering(offering *model.Offering) (string, error) {
	sqlStatement :=
		`INSERT INTO offerings(user_id, workspace_id, start_time, end_time, created_by, locked) VALUES ($1, $2, $3, $4, $5, false) RETURNING id`
	var id string
	err := p.database.QueryRow(sqlStatement,
		...
		offering.StartDate,
		offering.EndDate,
		offering.CreatedBy, // offering.Locked in creation set to false
	).Scan(&id)
	if err != nil {
		return "", err
	}
...
func (p PostgresDBStore) LockOffering(id string) error {
	sqlStatement :=
		`UPDATE offerings
				SET locked = true
				WHERE id = $1
...
func (p PostgresDBStore) UnlockOffering(id string) error {
	sqlStatement :=
		`UPDATE offerings
				SET locked = false
				WHERE id = $1
